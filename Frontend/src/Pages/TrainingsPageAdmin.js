...
export default function TrainingsPageAdmin() {
  ...
  const fecthTrainings = async () => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_URI}/training/training-cards`,
        {
          headers: {
            "x-access-token": user?.token,
          },
  ...
  const handleUpdate = (id, body) => {
    ...
    // Axios PUT request
    axios.put(
      `${process.env.REACT_APP_API_URI}/training/admin/update-training/${id}`,
      body,
      {
        headers: {
          "x-access-token": user?.token,
  ...
  const handleDeleteApi = async (val) => {
    try {
      const response = await axios.delete(
        `${process.env.REACT_APP_API_URI}/training/admin/delete-training/${val}`,
        {
          headers: {
            "x-access-token": user?.token,
  ...
  React.useEffect(() => {
    ...
    const fecthData = async () => {
      setIsLoading(true);
      await Promise.all([fecthTrainings(), getAllUserTrainings()]);
      setIsLoading(false);
    };
    fecthData();
  }, []);
  ...
  function handleSelect(value) {
    ...
    const newData = filteredData?.filter((item) => {
      if (selectedStatus === "All") {
        ...
        return (
          item.completedinprogress == selectedStatus.toString().toLowerCase()
        );
      }
    });
    setData(newData);
      ...
      <Box sx={{ display: "flex" }}>
        ...
        >
          <ConfirmDelete
            ...
            handleIncomeDelete={handleDeleteApi}
            id={id}
          />
          <Typography
            sx={{
              ...
              {courses?.map((course) => {
                return (
                  <TrainingCard
                    ...
                    handleDeleteApi={handleConfirmDelete}
